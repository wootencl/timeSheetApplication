...
function createUsersWithTimeSheets() {
  ...
  function insert(i) {
    if (i >= 0) {
      ...
      connection.query("SELECT REPLACE(UUID(),'-','') as token", function(err, results) {
        ...
        connection.query("INSERT INTO Persons Values(UNHEX(?), ?, ?, ?, ?, ?)", [token, user.LastName, user.FirstName, user.Email, user.Role, Password], function(err, results) {
          if(err) {
  ...
  function insertTimeSheet(token) {
    ...
    for (var j=5; j>0; j--) {
      ...
      connection.query("INSERT IGNORE INTO TimeSheets Values(UNHEX(?), ?, ?, ?, ?, ?, ?, ?)", [token, weekMonday, 5, j, 5, j, 5, (15+2*j)], function(err, results) {
        if(err) {
          console.log(err);
          return done();
        }
